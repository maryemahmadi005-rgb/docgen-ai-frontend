...
class Repository(db.Model, UUIDPKMixin, TimestampMixin):
    ...
    tracked_branch: Mapped[Optional[str]] = mapped_column(
        String(255),
        nullable=True,
    )

    local_clone_path: Mapped[Optional[str]] = mapped_column(
        String(500),
        nullable=True,
    )

    last_synced_commit_sha: Mapped[Optional[str]] = mapped_column(
        String(64),
        nullable=True,
    )
    ...
    sync_mode_updated_at: Mapped[Optional[datetime]] = mapped_column(
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "sync_method": (
                self.sync_method.value
                if self.sync_method
                else None
            ),
            "last_synced_commit_sha": self.last_synced_commit_sha,
            "current_readme_version_id": (
                self.current_readme_version_id
            ),
            "created_at": (
