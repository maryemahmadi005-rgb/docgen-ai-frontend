...
class PendingUpdateService:
    def __init__(self, pending_update_repository):
        """
        pending_update_repository : couche d'accès DB (injectée),
        expose create/get/list/update — pas de SQLAlchemy direct ici,
        pour garder ce service testable sans DB réelle.
        """
    ...
    # Lecture
    # ------------------------------------------------------------------
    def get(self, pending_update_id: str) -> PendingUpdate:
        pending_update = self.repo.get(pending_update_id)
        if pending_update is None:
            raise PendingUpdateNotFoundError(f"PendingUpdate {pending_update_id} introuvable.")
        return pending_update

    def list_for_repository(self, repository_id: str, status: str | None = None) -> list[PendingUpdate]:
        return self.repo.list_by_repository(repository_id, status=status)

    def get_active_pending(self, repository_id: str) -> PendingUpdate | None:
        """Retourne la proposition en statut 'pending' pour ce repo, s'il en existe une."""
        results = self.repo.list_by_repository(repository_id, status="pending")
        return results[0] if results else None

    # ------------------------------------------------------------------
    # Transitions d'état
    # ------------------------------------------------------------------
    def mark_approved(self, pending_update_id: str, user_id: str) -> PendingUpdate:
    ...
    def mark_rejected(self, pending_update_id: str, user_id: str, reason: str | None = None) -> PendingUpdate:
        pending_update = self._require_pending_status(pending_update_id)
        return self.repo.update(
            pending_update_id,
            status="rejected",
    ...
    def mark_stale(self, pending_update_id: str) -> PendingUpdate:
        """
    ...
    # Validation interne
    # ------------------------------------------------------------------
    def _require_pending_status(self, pending_update_id: str) -> PendingUpdate:
